...
public class FrontController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// post방식으로 들어오건 동일하게 doRequest로 처리하도록 설정
		doRequest(request,response);
	}
 // doGet, doPost 어떤 형식으로 요청이 들어올지 모르기 때문에 공용 메서드를 하단에 하나 생성합니다.
	private void doRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 프론트 컨트롤러 라는 여러 주소 요청을 처리하기 때문에 uri부터 받아옵니다.
		...
		if(uri.equals("/MyFirstWeb/boardList.do")) {
			...
			sv.execute(request,response);
			ui ="/boardDetail.do?boardnum=" + request.getParameter("board_num");
		}
			...
			// 그러면 정해진 주소 이외의 주소로 접속했을때 메인페이지로 보내주는 포워딩 구문을 작성.	
			ui = "/";
		}
