...
// DTO는 DB에서 꺼낸 데이터가 가공되어 전달될 때 쓰는것을 지칭할 때 씁니다.
// 다만 DTO와 VO를 엄격하게 구분하지않고 프로그래머에 따라서는
// 그냥 DTO를 VO와 완전히 동일하게 쓰는 경우도 있으므로
// 사용하는사람이 부르고 싶은대로 불러도 무방합니다.
public class BoardDTO {
	private int boardCount;// 전체 글개수
	private int currentPage; // 현재 보고있는 페이지
	private int totalPages; // 전체 페이지 개수
	private int startPage; // 시작 페이지 번호
	private int endPage; // 끝 페이지 번호
	private static final int NAV_NUM = 5;// 페이지는 10개씩 링크 표시(하단에 알림 버튼은 10개씩)
	private static final int BOARD_NUM = 20; // 글도 10개씩 표시
	
	// 생성자 성성시 모든 정보를 자동으로 구하도록 처리
	public BoardDTO(int boardCount, int currentPage) {
		...
		if(currentPage % NAV_NUM == 0) {
			...
			navNum = currentPage /NAV_NUM; // (51페이지 -1 / 10 해도 5가 나옴, 즉 50은 50-1/10 하면 4가나옴.)
		...
		// << 해당 조회중인 페이지 그룹의 끝페이지 구하기(endPage값 구하기)>>
